...
def plot_clustered_product_id_embedding(
    ...
    """
    对 Product ID embedding 进行 KMeans 聚类，并以聚类标签划分热力图，保存图片。
    """
    tok_dict = _load_tokenize_dict(json_path)
    base_name = 'Product ID'
    model_key = f'{base_name}_enc'
    if model_key not in model.cat_embs or base_name not in tok_dict:
        print(f"无法找到 Product ID embedding 或 tokenize_dict.json 无此特征。")
        return
    name_to_id = tok_dict[base_name]
    items = sorted(name_to_id.items(), key=lambda kv: kv[1])
    emb_layer = model.cat_embs[model_key]
    ...
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f'Product ID 聚类热力图已保存: {save_path}')


def plot_cluster_id_embedding(model, save_dir, max_tokens=60):
    """
    绘制 cluster_id embedding 的余弦相似度热力图。
    
    - model: 训练好的模型，包含 cluster_id embedding
    - save_dir: 保存图片的目录
    - max_tokens: 最大显示的 token 数量（默认60，对应 vocab_size）
    """
    ...
    if model_key not in model.cat_embs:
        print(f"模型中没有找到 {model_key} embedding。")
        return
    ...
    # 使用所有有效的 embedding（0 到 vocab_size-1）
    ...
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f'Cluster ID 热力图已保存: {save_path}')
...
def plot_clustered_cluster_id_embedding(
    ...
    """
    对 cluster_id embedding 进行 KMeans 聚类，并以聚类标签划分热力图，保存图片。
    """
    model_key = 'cluster_id'
    ...
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f'Cluster ID 聚类热力图已保存: {save_path}')
...
def main():
    ...
    parser.add_argument("--seed", type=int, default=42, 
                       help="Random seed")
    parser.add_argument('--product_id_clusters', type=int, default=8, help='Product ID embedding KMeans聚类数量')
    parser.add_argument('--cluster_id_clusters', type=int, default=8, help='Cluster ID embedding KMeans聚类数量')
